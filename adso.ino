...
 v1.0
(c) 2017    www.github.com/zooxo/adso

 

 ADSO is
a digital storage oscilloscope running on an arduino microcontroller.

 It samples,
stores and analyses an electrical signal.
...
 * Shows
signal graphically (80x64 pixel respective 10x8 divisions) as pixmap or polygon
...
#define DEFAULTSAMPLETIME 625 // default
sample time in us

#define SAMPLESIZE 160 // number of stored
values

#define SAMPLEDRAW 80 // number of drawn
values

#define SAMPLESPERDISPLAY 200 // display after
200 samples

#define DEFAULTSIGNALRATE 10000 // rate of
square reference signal in us

#define MAXMENUNR 6 // number of menus

#define DEFAULTTRIGLVL 25 // default trigger
level
...
#define TRIGLVLSTEP 4 // step change of
triglevel

#define XTEXT 81 // x-coordinate where text
starts
...
#define EEPROMOFFSET 64 // eeprom adress where
sample is stored

#define FCNR 10 //number of fourier
coefficients
...
boolean isreference=true; // reference signal
(signalpin) on

boolean isground=false; // no grounding of
signal
...
byte pin=1,pwmpin=11,signalpin=9; // analog
read pin, PWM- and square-reference pin

char key; // scanned key

byte menunr=1; // default menu number

byte val[SAMPLESIZE],ptr=0; // sampled value
storage

char xoffset=0,yoffset=0; // offset variables

byte triglevel=DEFAULTTRIGLVL; // trigger
level

byte displaycounter=0; // counts scans and
display every SCANSPERDISPLAY times

double a[FCNR]; // fourier coefficients

byte nstrip; // number of sample values used
for fourier

unsigned long scanstart=0; // used for square
reference 

unsigned long
signaltime=DEFAULTSIGNALRATE,signalmicros=0;
...
#define numberofvoltsteps
(sizeof(voltsteps)/sizeof(const int *))

byte vsnr=4; // volt step index (default 4 =
2000 mV/div)
...
#define numberofsampletimes
(sizeof(sampletimes)/sizeof(const int *))

byte stnr=2; // sample time index (default 2 =
625 ms/div)

unsigned long timestamp=0,startmicros=0; //
used for sample time measurement

unsigned long
sampletime=sampletimes[stnr],sampledtime=0;
...
void curpos(byte x, byte y, byte font) { //
...
void fourier() { // calculate fourier
coefficients

  byte
trigpnt1=0,trigpnt2=0;
...
     
if(isdrawlines)
display.drawLine(i+xoffset,val[trigptr],i+1+xoffset,val[trigptr+1],WHITE);
...
 
if(voltsteps[vsnr]<1000)
display.print((double)(voltsteps[vsnr]/1000.0),1);

  else
display.print(voltsteps[vsnr]/1000);
...
void reset() { // reset adjustments to default
values
...
void save() { // saves adjustments and sample
values
...
void load() { // loads adjustments and sample
values
...
 
if(timestamp-startmicros>=sampletime) { // measurement if desired
sampletime is reached
...
    //
k   = 8pxperdiv / Vpd / ((1024analogvals
- 512valforzero) / (10xdiv / 2ranges))
...
     
if(displaycounter>=SAMPLESPERDISPLAY/sampletimes[stnr]) { // print
screen
